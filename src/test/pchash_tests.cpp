...
namespace {
    ...
    bool TapscriptCheck(const valtype& witVerifyScript, const std::vector<valtype>& witData)
    {
        ...
        script_verify_flags flags = SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_TAPROOT | SCRIPT_VERIFY_PAIRCOMMIT;
		...
		auto script_check_error = CScriptCheck(txFrom.vout[0], CTransaction(txTo), signature_cache, 0, flags, false, &txdata)();

        return !script_check_error.has_value();
    }
}
